...
int n, a, b, w[MAX], q, u, v;
int cur, h[MAX], nxt[MAX], p[MAX];
int siz[MAX], top[MAX], son[MAX], dep[MAX], fa[MAX], dfn[MAX], rnk[MAX],cnt;
char op[10];
...
int main() {
  ...
  while (q--) {
    ...
    if (!strcmp(op, "QMAX")) cout<<querymax(u, v)<<endl;
    if (!strcmp(op, "QSUM")) cout<<querysum(u, v)<<endl;
  }
  return 0;
}